	...
	private void writeItem(JsonItem item, boolean first) throws IOException {
		...
		default:
			...
			if (name == null || name.length() == 0) {
				...
				jsonGen.writeStringField("type", jsonType.typeName);
				jsonGen.writeNumberField("position", item.getPosition());
				jsonGen.writeNumberField("storageLength", item.getStorageLength());
				jsonGen.writeNumberField("displayLength", item.getDisplayLength());
